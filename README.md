...
## Devlog Entry - [12/8/25]

## How we satisfied the software requirements 3

1. We implemented a system to detect the current pages settings of whether or not the page is using light or dark settings to change the appearance of the level and some of the elements in the level. This also includes an ability for it to update if you update your browser preference in the window.
2. We implemented a new file called touchScreenControls.ts that handles touchscreen implementation by creating a joystick that can appear on the left side of the screen along with camera control on the left side. Then we used inputs from the joystick to replace keytaps and created a ui button in the right bottom corner that allows for in-game interactions with objects such as the rope cutting feature.
3. We also added in support for i18n + l10n using the translation.ts file. There along with 3 language files we created in the locales folder located in src, allows for the capability to switch between languages as you need, english being the default language. Then we implemented a ui selecter bar to allow for one to choose their preferred language. It does these updates where it will remove anything with wording on it and then regenerate it all with the correct language, doing this on each and every language swap. The languages we chose to use were Japanese and Arabic and we utilized Google Translate for the translations.
4. We finally implemented in the ability to do unlimited undos by simply readjusting an already existing function in the code. The function: resetSimulation, already reset the rope and ball and placed everything back as it should be so we decided the best way to implement unlimited undos would to add more conditions into this function to allow it to back pedal back from important moments, especially considering there isn't all too much to do (yet).

## Reflection 3

Our thinking has changed as we attempted some of the other implementations but realized that the ball and rope idea, although good and simple in practice, makes some of the conditions very limiting such as continous inventory. There was also the fact that some were simply to difficult to understand how to implement or would require large parts of the code to change which would take even longer to work through. But overall, this additional implementation of extra features helped us to broaden the horizen of our games as it shows that just because a game has the basics, doesn't mean there can't be more added on to it. A good example is the translation, we never thought about trying to learn how to make it possible to switch between languages and now that we have that experience, it will be interesting on how we can utilize this later.
